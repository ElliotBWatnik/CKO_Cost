...
# --- UPDATED GRANULAR SUMMARY FUNCTION ---
def build_granular_summary_df(df_subset, scheme_name):
    if df_subset.empty:
        return pd.DataFrame()
        
    # Group by the specific granular dimensions
    ...
    # 3. Calculate Categorical Fees (Interchange, Gateway, etc.)
    cat_fees = df_subset.groupby(group_cols + ['Fee Category'])['Absolute Fee Cost'].sum().reset_index()
    
    # Pivot so Fee Categories become columns using both Region and Card Type as the index
...
# Loop through Visa and Mastercard and combine results into one table
summary_dfs = []
