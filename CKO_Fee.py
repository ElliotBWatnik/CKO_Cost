...
def load_data(file_bytes):
    # Read from bytes to avoid Streamlit file pointer/caching issues
    ...
    if 'Total Amount in Holding Currency' in df.columns:
        ...
        # Fallback if the column is named slightly differently
        st.error("Missing required column: 'Total Amount in Holding Currency'. Please ensure you uploaded the Fee Report and not the Payments Report.")
        st.stop()
...
# Read the file to bytes before passing to the cached function
try:
...
if 'Breakdown Type' in filtered_df.columns:
...
if 'Breakdown Type' in filtered_df.columns:
    ...
    with st.expander("View Detailed Breakdown by Fee Detail"):
        if 'Fee Detail' in filtered_df.columns:
